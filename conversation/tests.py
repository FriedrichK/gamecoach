...
class ToolsFolderTestCase(TestCase):
    ...
    def test_returns_empty_inbox_slice_in_deleted_mode(self):
        user1 = self._get_user(0)
        ...
        inbox = get_inbox_slice(user1, time_anchor, older=True, items=5, deleted=True)
        actual = [message.sender.id for message in inbox]

        self.assertEqual(actual, [])

    def test_returns_expected_inbox_slice_in_deleted_mode_if_user_can_moderate(self):
        user1 = self._get_user(0)
        user1.is_superuser = True
        fake_conversation_batched, fake_conversation_flat = self._create_fake_conversation(items=1)
